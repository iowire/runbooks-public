...
- `celery-beat` — scheduled tasks: nightly Connect reconciliation (`06:00 UTC`), nightly Stripe transfer reconciliation, sandbox balance topup every 4h at `:15` (`tasks.sandbox_topup.maintain_sandbox_balance`, no-op outside sandbox mode). Full schedule: `boom-queue/celery_app.py` `beat_schedule`.
  - **`sweep-declined-payment-notifications`** (T3, `tasks.decline_notifier`, `bulk` queue, every 5 min) — deferred decline notifier. **Dark by default on prod AND staging**: the beat entry fires but the task returns `{flag_off: true}` and touches nothing until the `decline_notifier` flag is enabled per-env (`FF_DECLINE_NOTIFIER=true`). Read-only re Stripe (retrieves PIs; writes only `Notification` + `ProcessedWebhookEvent` rows — never charges/refunds/captures/cancels). When enabled it selects interactive-checkout payments that failed 30+ min ago, re-reads each PI, and notifies only the ones that did NOT recover. Enable procedure (incl. the mandatory `DECLINE_NOTIFY_NOT_BEFORE` storm guard): `docs/deployment/POST_CUT_FLAG_FLIP_RUNBOOK.md`. To halt an *enabled* sweep immediately, set `FF_DECLINE_NOTIFIER=false` + config-redeploy, or stop `celery-beat` as a stopgap.
